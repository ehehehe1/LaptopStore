    ...
    if ($result) {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $imageSrc = $row['IMG'] ? '/LaptopStore/Store/assets/img/product/' . htmlspecialchars($row['IMG']) : '/LaptopStore/Store/assets/img/product/default-product.jpg';
                $html .= '
                    <a href="/LaptopStore/Store/layout/chitietsp.php?masp=' . htmlspecialchars($row['MASP']) . '" class="product-link" style="text-decoration: none; color: inherit;">
                        <div class="product">
